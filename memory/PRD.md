...
## Itération 6 (24/09/2026) — restructuration GitHub Pages à la racine
Contenu de static_site/ déplacé à la racine du dépôt via git mv (commit c07405d sur main) : index.html, 5 pages, 404.html, favicon.svg, css/, js/, assets/, documents/, README.md (procédure Pages) tous à la racine ; dossier static_site supprimé (0 fichier restant) ; README template racine remplacé. Vérifié : git ls-files montre index.html à la racine, site servi depuis la racine → 8/8 URL en 200 (index, css, js, logo, 2 plaquettes, 404). Le dépôt synchronise main → GitHub comme les commits précédents.

## Itération 5 (24/09/2026) — version statique GitHub Pages
Dossier prêt à pousser : /app/static_site/ (index.html à la racine + entreprise/parcours/catalogue/fabricants/contact/404 .html, css/style.css, js/main.js, assets/ images+polices locales, documents/ 6 PDF, favicon.svg, README.md avec procédure Settings → Pages → Deploy from a branch → main → root). Aucun backend : formulaire de devis en mailto pré-rempli vers snpariscm@gmail.com ; visionneuse PDF = lecteur natif du navigateur (iframe + fragments #page/#zoom, bouton « Ouvrir le PDF ») ; carte OSM en iframe. Chemins 100 % relatifs, aucun localhost. Pagination des renvois catalogue corrigée partout (parcours statique + React content.js) sur la nouvelle plaquette complète (famille 01 → p. 7-8, etc.).
Vérifié : 20 URL en 200 depuis un sous-dossier simulé /nom-du-depot/ (index, css, js, images, fonts, 6 PDF, 5 pages, 404, favicon) ; grep : aucun chemin absolu ni localhost ; screenshots 375/768/1366 px (accueil, téléchargements, catalogue avec navigation page 17/37 puis plaquette courte 2/8, contact avec carte OSM + modale devis).
